...
<script setup>
	...
	const htmlSerializer ={
		...
		heading2:({children})=> `<h2 class="font-header font-bold leading-1.2 my-1.6 first:mt-0 last:mb-0 text-2.6 d:text-2.8">${formatChildren(children)}</h2>`,
		heading3:({children})=> `<h3 class="font-header font-bold leading-1.1 my-1.6 first:mt-0 last:mb-0 text-2.4 d:text-2.5">${formatChildren(children)}</h3>`,
		...
		paragraph:({children})=> `<p class="text-slate-500 text-1 m:text-1.2 leading-1.4 my-0.5 first:mt-0 last:mb-0">${formatChildren(children)}</p>`,
